...
def create_verification_request(
    ...
    if payload.delivery_method == VerificationDeliveryMethod.VERIFIER and not payload.verifier_email:
        raise HTTPException(
...
def generate_verification_letter(
    ...
    if payload.delivery_method == VerificationDeliveryMethod.VERIFIER and not request.verifier_email:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Verifier email is required for direct delivery",
        )
    ...
    if request.verifier_email:
        metadata["verifier_email"] = request.verifier_email
    try:
        upload_pdf_bytes(s3_key, pdf_bytes, metadata=metadata)
    except S3ConfigError as exc:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(exc)) from exc

    if payload.delivery_method == VerificationDeliveryMethod.EMPLOYEE:
        document = db.get(Document, request.document_id) if request.document_id else None
        document_title = (
            f"Employment Verification Letter - {generated_at_local.strftime('%Y-%m-%d')}"
        )
        verifier_email = request.verifier_email or "no email provided"
        document_body = "\n".join(
            [
                "Employment verification letter generated by HR.",
                f"Employee: {employee.legal_first_name} {employee.legal_last_name}",
                f"Verifier: {request.verifier_name} ({verifier_email})",
                f"Generated on: {generated_at_local.strftime('%B %d, %Y')}",
                f"Purpose: {request.purpose}",
                "Download the PDF for the signed letter.",
            ]
        )
        if not document:
            document = Document(
    ...
    request.delivery_method = payload.delivery_method
    db.commit()
    db.refresh(request)
...
def mark_verification_sent(
    ...
    if request.delivery_method == VerificationDeliveryMethod.EMPLOYEE:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
